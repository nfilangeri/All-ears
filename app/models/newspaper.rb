...
class Newspaper < ApplicationRecord
  ...
  def self.get_articles_bbc
    ...
    list_of_articles.each do |a|
      article = Article.create(title: a["title"], description: a["description"], content: a["body"], publication_date: a["datePublished"], image: a["image"]["url"], newspaper_id: 1)
    end
  end
end
